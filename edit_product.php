<?php
	...
	if(!isset($_SESSION['id']))
	{
		...
		$execute=array();
		$exe_find_pic=find("first",ITEM,"*", "where id=".base64_decode($_GET['eid']), $execute);
	
		if(isset($_POST['save']))
		{
			$execute=array();
			$exe_find=find("first",ITEM,"*", "where code='".$_POST['p_code']."' and id!=".base64_decode($_GET['eid']), $execute);
			if (!$exe_find) 
			{
				$p_code=$_POST['p_code'];
				$p_desc=$_POST['p_desc'];
				...
				if(isset($_FILES['img']['name']) && ($_FILES['img']['name'])!='')
				{
					...
					$set_value="code=:code,image=:image,description=:description,price=:price";
					$where_clause="where id=:id";
					$execute=array(':id'=>$id, ':code'=>$p_code, ':image'=>$randimage, ':description'=>$p_desc,':price'=>$p_price);
					$sv_mchne=update(ITEM, $set_value, $where_clause, $execute);
					
				}
				else
				{
					$set_value="code=:code,description=:description,price=:price";
					$where_clause="where id=:id";
					$execute=array(':id'=>$id,':code'=>$p_code,':description'=>$p_desc,':price'=>$p_price);
					$sv_mchne=update(ITEM, $set_value, $where_clause, $execute);
				}
				...
				if($sv_mchne)
				{
				 ...
				 $_SESSION['SET_FLASH'] = 'Product Updated successfully';
					  ...
					  <div class="col-md-12">
					  	  ...
					  	  <div class="col-md-6">
						  	  <div class="form-group">
									<img src="item_img/<?php echo $exe_find_pic['image']?>" style="border-radius:10%; width: 100px; height: 100px;"> 
							  </div>
						  	  <div class="form-group">
						  	  	<label>Product Code</label>
								<input type="text" class="form-control" id="p_code" name="p_code" placeholder="Product Code" value="<?php echo $exe_find_pic['code']?>"  required>
							  </div>
					  	  	  
						  	  <div class="form-group" >
						  	  	<label>Product Description</label>
								<input type="text" class="form-control" id="p_desc" name="p_desc" placeholder="Description" value="<?php echo $exe_find_pic['description']?>"  required>
					
							  </div>
							  <div class="form-group">
							  	<label>Price</label>
								<input type="text" class="form-control" id="p_price" name="p_price" placeholder="Price" value="<?php echo $exe_find_pic['price']?>"  required>
